...
class FeatureDiversifier:
    ...
    def apply_interaction_dropout(
        ...
        """
        _abort_random_augmentation("FeatureDiversifier.apply_interaction_dropout")
    ...
    def apply_adversarial_scramble(
        self, features: np.ndarray, epoch: int, batch: int
    ) -> np.ndarray:
        """
        ...
        """
        _abort_random_augmentation("FeatureDiversifier.apply_adversarial_scramble")
    ...
    def apply_mixup(
        self, features: np.ndarray, labels: np.ndarray, epoch: int, batch: int
    ):
        """Mixup augmentation with lambda ~ Beta(alpha, alpha)."""
        _abort_random_augmentation("FeatureDiversifier.apply_mixup")
    ...
    def apply_noise_augmentation(
        ...
        _abort_random_augmentation("FeatureDiversifier.apply_noise_augmentation")
    ...
    def compute_interaction_contribution(self, features: np.ndarray) -> float:
        ...
        if total_var < 1e-10:
            return 0.0
        interaction_var = variances[
            self.config.interaction_start : self.config.interaction_end
        ].sum()
...
class DriftAugmenter:
    ...
    def apply_domain_randomization(
        features: np.ndarray, scale_pct: float = 0.10, seed: int = 42
    ) -> np.ndarray:
        """Apply ±scale% feature scaling per dimension."""
        _abort_random_augmentation("DriftAugmenter.apply_domain_randomization")
    ...
    def apply_random_missingness(
        features: np.ndarray, miss_rate: float = 0.10, seed: int = 42
    ) -> np.ndarray:
        """Replace random features with neutral value 0.5."""
        _abort_random_augmentation("DriftAugmenter.apply_random_missingness")
        rng = np.random.RandomState(seed)
    ...
    @staticmethod
    def inject_novel_patterns(
        features: np.ndarray, inject_rate: float = 0.05, seed: int = 42
    ) -> np.ndarray:
        """Inject novel structural patterns into fraction of batch."""
        _abort_random_augmentation("DriftAugmenter.inject_novel_patterns")
        rng = np.random.RandomState(seed)
        result = features.copy()
        ...
        result[:n_inject, 192:256] = np.clip(
            rng.normal(0.5, 0.15, (n_inject, 64)), 0, 1
        )

        # Small perturbation to interaction dims
        perturbation = (rng.random((n_inject, 64)) - 0.5) * 0.1
        result[:n_inject, 128:192] = np.clip(
            result[:n_inject, 128:192] + perturbation, 0, 1
        )
    ...
    def apply_correlated_noise(
        features: np.ndarray, sigma: float = 0.03, seed: int = 42
    ) -> np.ndarray:
        """Correlated noise across dims."""
        _abort_random_augmentation("DriftAugmenter.apply_correlated_noise")
        rng = np.random.RandomState(seed)
        result = features.copy()
