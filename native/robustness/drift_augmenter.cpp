...
void apply_domain_randomization(float* features, int batch_size, int dim,
    ...
    std::uniform_real_distribution<float> scale(1.0f - scale_pct, 1.0f + scale_pct);
    
    for (int d = 0; d < dim; ++d) {
        float s = scale(rng);
        for (int b = 0; b < batch_size; ++b) {
            features[b * dim + d] = std::max(0.0f, std::min(1.0f, features[b * dim + d] * s));
        }
    }
}
...
void inject_novel_patterns(float* features, int batch_size, int dim,
    ...
    for (int b = 0; b < n_inject; ++b) {
        ...
        for (int d = 128; d < std::min(dim, 192); ++d) {
            ...
            features[b * dim + d] = std::max(0.0f, std::min(1.0f, features[b * dim + d]));
        }
    }
}
...
void apply_correlated_noise(float* features, int batch_size, int dim,
    ...
    for (int b = 0; b < batch_size; ++b) {
        ...
        for (int d = 0; d < dim; ++d) {
            ...
            features[b * dim + d] = std::max(0.0f, std::min(1.0f, features[b * dim + d] + n));
        }
    }
}
