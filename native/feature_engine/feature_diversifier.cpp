...
void FeatureDiversifier::apply_interaction_dropout(
    float* features, int batch_size, int dim, uint64_t epoch, uint64_t batch) {
    
    if (!config_.training) return;
...
void FeatureDiversifier::apply_adversarial_scramble(
    float* features, int batch_size, int dim, uint64_t epoch, uint64_t batch) {
    ...
    if (uniform(local_rng) > 0.10f) return;
...
void FeatureDiversifier::expand_independent_channels(
    const float* input, float* output, int batch_size, int input_dim) {
...
void FeatureDiversifier::apply_mixup(
    float* features, float* labels, int batch_size, int dim,
    uint64_t epoch, uint64_t batch) {
    
    if (!config_.training || batch_size < 2) return;
    ...
    for (int b = 0; b < batch_size / 2; ++b) {
        ...
        for (int d = 0; d < dim; ++d) {
            float mixed = lam * features[i * dim + d] + (1.0f - lam) * features[j * dim + d];
...
void FeatureDiversifier::apply_noise_augmentation(
    float* features, int batch_size, int dim, uint64_t epoch, uint64_t batch) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        ...
        // Apply noise to signal and response dims only (NOT interaction — break dependency)
        for (int d = config_.signal_start; d < config_.signal_end; ++d) {
...
float FeatureDiversifier::compute_interaction_contribution(
    ...
    for (int d = 0; d < dim; ++d) means[d] /= batch_size;
...
float FeatureDiversifier::compute_balance_penalty(
    const float* features, int batch_size, int dim) {
