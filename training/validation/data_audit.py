...
def _mutual_information(
    features: np.ndarray, labels: np.ndarray, n_bins: int = 20
) -> float:
    """Estimate mutual information between features (1D) and binary labels."""
    # Discretize features
    feat_bins = np.digitize(
        features, np.linspace(features.min(), features.max(), n_bins)
    )
...
def run_data_audit(
    ...
    overlap_risk = (
        "LOW" if signal_gap > 0.2 else ("MEDIUM" if signal_gap > 0.1 else "HIGH")
    )
    ...
    section_a = {
        ...
        else (1.0 if dataset_source == "SYNTHETIC_GENERATOR" else None),
    ...
    for start, end, name in groups:
        group_stats[name] = {
            ...
            "mean_diff": round(
                float((pos_mean[start:end] - neg_mean[start:end]).mean()), 4
            ),
        }
    ...
    for start, end, name in groups:
        pos_hist, _ = np.histogram(
            features[pos_mask, start:end].flatten(), bins=50, density=True
        )
        neg_hist, _ = np.histogram(
            features[neg_mask, start:end].flatten(), bins=50, density=True
        )
...
def generate_audit_report(result: AuditResult) -> str:
    ...
    for i, feat in enumerate(c.get("top_10_correlated_features", [])):
        lines.append(
            f"    #{i + 1:2d}: dim={feat['dim']:3d} corr={feat['correlation']:+.4f} ({feat['group']})"
        )
...
if __name__ == "__main__":
    ...
    if STRICT_REAL_MODE:
        ...
        raise RuntimeError(
            "REAL_DATA_REQUIRED: Synthetic validation datasets are disabled for data_audit"
        )
