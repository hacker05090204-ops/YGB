 ...
 */
static const std::set<std::string> FORBIDDEN_FIELDS = {"valid",
                                                       "accepted",
                                                       "rejected",
                                                       "severity",
                                                       "decision",
  ...
  for (unsigned char ch : value) {
    ...
    default:
      if (ch < 0x20) {
        ...
        unicode_escape << "\\u" << std::hex << std::uppercase
                       << static_cast<int>(ch);
...
std::string utc_timestamp_now() {
  ...
  if (gmtime_s(&utc_tm, &now) != 0) {
    throw std::runtime_error("gmtime_s failed while building UTC timestamp");
  }
#else
  if (gmtime_r(&now, &utc_tm) == nullptr) {
    throw std::runtime_error("gmtime_r failed while building UTC timestamp");
  }
#endif
  char buffer[64];
  if (std::strftime(buffer, sizeof(buffer), "%Y-%m-%dT%H:%M:%SZ", &utc_tm) ==
      0) {
    throw std::runtime_error("strftime failed while building UTC timestamp");
  }
...
DOMElement extract_dom_structure(const std::string &html_content) {
  ...
  while (pos < html_content.size()) {
    ...
    if (!tag_content.empty() && tag_content[0] != '/' &&
      ...
      std::transform(child.tag.begin(), child.tag.end(), child.tag.begin(),
                     ::tolower);
 ...
 */
HTTPTrace
capture_http_trace(const std::string &method, const std::string &url,
                   int status_code,
                   const std::map<std::string, std::string> &req_headers,
                   const std::map<std::string, std::string> &resp_headers,
                   const std::string &body, double response_time_ms) {
  HTTPTrace trace;
  trace.method = method;
  trace.url = url;
 ...
 */
std::map<std::string, std::string> normalize_response(const HTTPTrace &trace) {
  ...
  for (const auto &[key, value] : trace.response_headers) {
    ...
    if (lower_key == "content-type" || lower_key == "content-length" ||
        lower_key == "server" || lower_key == "x-frame-options" ||
 ...
 */
SanitizedRecord
sanitize_output(const std::string &id, const std::string &source_url,
  ...
  // Get current timestamp using a thread-safe UTC conversion.
  record.timestamp = utc_timestamp_now();
 ...
 */
bool write_sanitized_record(const SanitizedRecord &record,
  ...
  // Write minimal JSON with escaping for every user-controlled string.
  file << "{\n";
  file << "  \"id\": \"" << escape_json(record.id) << "\",\n";
  file << "  \"timestamp\": \"" << escape_json(record.timestamp) << "\",\n";
  file << "  \"source_url\": \"" << escape_json(record.source_url) << "\",\n";
  file << "  \"sanitized\": true,\n";
  file << "  \"dom_tags\": [";
  for (size_t i = 0; i < record.dom_tags.size(); i++) {
    if (i > 0)
      file << ", ";
    file << "\"" << escape_json(record.dom_tags[i]) << "\"";
  }
  file << "],\n";
  file << "  \"http_metadata\": {\n";
  size_t count = 0;
  for (const auto &[key, value] : record.http_metadata) {
    if (count > 0)
      file << ",\n";
    file << "    \"" << escape_json(key) << "\": \"" << escape_json(value)
         << "\"";
    count++;
  }
  file << "\n  }\n";
