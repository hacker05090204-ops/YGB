...
def _install_fake_storage(users):
    ...
    original_google_index = dict(storage_bridge._GOOGLE_SUB_INDEX)
    original_google_built = storage_bridge._GOOGLE_SUB_INDEX_BUILT
    ...
    storage_bridge._GOOGLE_SUB_INDEX = {}
    storage_bridge._GOOGLE_SUB_INDEX_BUILT = False
    ...
    def restore():
        ...
        storage_bridge._GOOGLE_SUB_INDEX = original_google_index
        storage_bridge._GOOGLE_SUB_INDEX_BUILT = original_google_built
...
def test_update_user_auth_profile_promotes_google_identity_fields():
    users = {"user-1": _base_user()}
    restore = _install_fake_storage(users)
    try:
        storage_bridge.update_user_auth_profile(
            ...
            google_profile={
                ...
                "google_email": "octocat@gmail.com",
                "google_picture": "https://lh3.googleusercontent.com/a/avatar",
            },
        )

        record = storage_bridge.get_user("user-1")
        assert record["auth_provider"] == "google"
        assert record["google_sub"] == "google-sub-1"
        assert record["google_email"] == "octocat@gmail.com"
        assert record["google_picture"] == "https://lh3.googleusercontent.com/a/avatar"
        assert record["avatar_url"] == "https://lh3.googleusercontent.com/a/avatar"
    finally:
        restore()


def test_get_user_by_google_sub_uses_stable_identity_lookup():
    users = {
        "user-1": {
            **_base_user(),
            "auth_provider": "google",
    ...
    restore = _install_fake_storage(users)
    try:
        record = storage_bridge.get_user_by_google_sub("google-sub-1")
        assert record is not None
        assert record["id"] == "user-1"
        assert record["google_email"] == "octocat@gmail.com"
    finally:
...
def test_resolve_or_create_google_user_prefers_google_sub(monkeypatch):
    google_user = {"id": "user-google", "email": "new@example.com", "role": "hunter"}
    email_user = {"id": "user-email", "email": "new@example.com", "role": "hunter"}
    created = []

    monkeypatch.setattr(server_mod, "get_user_by_google_sub", lambda google_sub: google_user)
    monkeypatch.setattr(server_mod, "get_user_by_email", lambda email: email_user)
    monkeypatch.setattr(
        server_mod,
        "create_user",
    ...
    record, is_new_user = server_mod._resolve_or_create_google_user(
        google_sub="google-sub-1",
        effective_email="new@example.com",
        display_name="Octocat Google",
    )

    assert record["id"] == "user-google"
    assert is_new_user is False
    assert created == []
...
def test_auth_me_returns_google_fields_from_storage(monkeypatch):
    monkeypatch.setattr(
        server_mod,
        "get_user",
        lambda user_id: {
    ...
    assert payload["google_email"] == "octocat@gmail.com"
    assert payload["google_picture"] == "https://lh3.googleusercontent.com/a/avatar"
    assert payload["avatar_url"] == "https://lh3.googleusercontent.com/a/avatar"
...
def test_google_oauth_runtime_config_reports_missing_fields(monkeypatch):
    monkeypatch.setenv("GOOGLE_CLIENT_ID", "")
    monkeypatch.setenv("GOOGLE_CLIENT_SECRET", "")
    monkeypatch.setenv(
        "GOOGLE_REDIRECT_URI",
        "http://192.168.1.10:8000/auth/google/callback",
    )
    monkeypatch.setenv("FRONTEND_URL", "http://192.168.1.10:3000")

    detail = server_mod._oauth_not_configured_detail("google")

    assert detail["error"] == "GOOGLE_OAUTH_NOT_CONFIGURED"
    assert "GOOGLE_CLIENT_ID" in detail["missing"]
    assert "GOOGLE_CLIENT_SECRET" in detail["missing"]
    assert detail["redirect_uri"] == "http://192.168.1.10:8000/auth/google/callback"
...
def test_auth_provider_status_uses_shared_server_side_oauth(monkeypatch):
    ...
    assert payload["google"]["missing"] == []
    assert payload["google"]["redirect_uri"] == "http://192.168.1.10:8000/auth/google/callback"
    assert payload["google"]["frontend_url"] == "http://192.168.1.10:3000"
