"""Phase-20 Forbidden Import Tests."""
import pytest
from pathlib import Path

PHASE20_DIR = Path(__file__).parent.parent
IMPLEMENTATION_FILES = ["phase20_types.py", "phase20_context.py", "phase20_engine.py", "__init__.py"]


def _read_source(filename: str) -> str:
    filepath = PHASE20_DIR / filename
...
class TestForbiddenImports:
    ...
    def test_no_future_phase_imports(self, filename: str) -> None:
        ...
        for phase in range(21, 35):
            assert f"phase{phase}" not in source


class TestNoFileOperations:
    @pytest.mark.parametrize("filename", ["phase20_types.py", "phase20_context.py", "phase20_engine.py"])
