"""Phase-24 Forbidden Import Tests."""
import pytest
from pathlib import Path

PHASE24_DIR = Path(__file__).parent.parent
IMPLEMENTATION_FILES = ["phase24_types.py", "phase24_context.py", "phase24_engine.py", "__init__.py"]


def _read_source(filename: str) -> str:
    filepath = PHASE24_DIR / filename
...
class TestForbiddenImports:
    ...
    def test_no_future_phase_imports(self, filename: str) -> None:
        ...
        for phase in range(25, 35):
            assert f"phase{phase}" not in source


class TestNoFileOperations:
    @pytest.mark.parametrize("filename", ["phase24_types.py", "phase24_context.py", "phase24_engine.py"])
