...
CVE_PATTERNS = {
    ...
    "jquery": [
        {
            "pattern": r"jquery[/-]?v?1\.[0-9]+",
            "cve": "CVE-2020-11022",
            "severity": "MEDIUM",
            "cvss": 6.1,
            "title": "jQuery < 3.5.0 XSS Vulnerability",
        ...
        {
            "pattern": r"jquery[/-]?v?2\.[0-2]",
            "cve": "CVE-2020-11023",
            "severity": "MEDIUM",
            "cvss": 6.1,
            "title": "jQuery < 3.5.0 XSS in htmlPrefilter",
            "desc": "Passing HTML containing <option> from untrusted sources may execute untrusted code",
            "remediation": "Upgrade jQuery to version 3.5.0 or later",
        },
        {
            "pattern": r"jquery[/-]?v?3\.[0-4]\.[0-9]",
            "cve": "CVE-2019-11358",
            "severity": "MEDIUM",
            "cvss": 6.1,
    ...
    "angular": [
        {
            "pattern": r"angular[.-]?1\.[0-5]",
            "cve": "CVE-2022-25869",
            "severity": "HIGH",
            "cvss": 7.5,
            "title": "AngularJS XSS Vulnerability",
        ...
        {
            "pattern": r"angular[.-]?1\.[6-9]",
            "cve": "CVE-2020-7676",
            "severity": "MEDIUM",
            "cvss": 6.1,
            "title": "AngularJS Prototype Pollution",
    ...
    "react": [
        {
            "pattern": r"react[/-]?v?16\.[0-3]",
            "cve": "CVE-2018-6341",
            "severity": "MEDIUM",
            "cvss": 5.4,
            "title": "React XSS in SSR",
    ...
    "bootstrap": [
        ...
        {
            "pattern": r"bootstrap[/-]?v?4\.[0-2]",
            "cve": "CVE-2018-14041",
            "severity": "MEDIUM",
            "cvss": 6.1,
            "title": "Bootstrap tooltip XSS",
    ...
    "moment": [
        {
            "pattern": r"moment[/-]?v?2\.[0-9]+",
            "cve": "CVE-2022-31129",
            "severity": "HIGH",
            "cvss": 7.5,
            "title": "Moment.js ReDoS Vulnerability",
    ...
    "wordpress": [
        ...
        },
        {
            ...
            "severity": "MEDIUM",
            "cvss": 5.4,
            "title": "WordPress Directory Traversal",
    ...
    "apache": [
        ...
        },
        {
            ...
            "severity": "HIGH",
            "cvss": 7.5,
            "title": "Apache mod_ssl NULL Pointer",
...
HEADER_CVE_PATTERNS = {
    "server": [
        ...
        },
        {
            ...
            "severity": "HIGH",
            "cvss": 7.5,
            "title": "nginx Chunked Transfer Coding Stack Overflow",
        },
        {
    ...
    "x-powered-by": [
        {
            ...
            "severity": "CRITICAL",
            "cvss": 9.8,
            "title": "PHP 5.x End of Life - Multiple CVEs",
        },
        {
            "pattern": r"PHP/7\.[0-3]",
            "cve": "CVE-2019-11043",
            "severity": "HIGH",
            "cvss": 9.8,
            "title": "PHP-FPM RCE Vulnerability",
        },
        {
            "pattern": r"Express",
            "cve": "CVE-2022-24999",
            "severity": "MEDIUM",
            "cvss": 5.3,
            "title": "Express.js Information Disclosure",
        },
        {
            "pattern": r"ASP\.NET",
            "cve": "CVE-2021-26855",
            "severity": "HIGH",
            "cvss": 9.8,
            "title": "Check for Exchange ProxyLogon",
        },
    ],
}

COMPILED_CVE_PATTERNS = {
    tech: [
        {**pattern_info, "_compiled": re.compile(pattern_info["pattern"])}
        for pattern_info in patterns
    ]
    for tech, patterns in CVE_PATTERNS.items()
}

COMPILED_HEADER_CVE_PATTERNS = {
    header: [
        {**pattern_info, "_compiled": re.compile(pattern_info["pattern"], re.I)}
        for pattern_info in patterns
    ]
    for header, patterns in HEADER_CVE_PATTERNS.items()
}

SCRIPT_SOURCE_PATTERN = re.compile(r'<script[^>]+src=["\']([^"\']+)["\']', re.I)
VULNERABLE_SCRIPT_PATTERNS = [
...
class CVEScanner:
    ...
    async def _emit_finding(self, match: CVEMatch):
        """Emit a CVE finding."""
        fingerprint = (
            f"{match.cve_id}:{match.affected_component}:{match.detection_method}"
        )
    ...
    async def scan_content(self, content: str, url: str = "") -> List[CVEMatch]:
        ...
        for tech, patterns in COMPILED_CVE_PATTERNS.items():
            for pattern_info in patterns:
                if pattern_info["_compiled"].search(content_lower):
                    match = CVEMatch(
    ...
    async def scan_headers(
        ...
        for header_name, patterns in COMPILED_HEADER_CVE_PATTERNS.items():
            header_value = headers_lower.get(header_name, "")
            if not header_value:
                continue

            for pattern_info in patterns:
                if pattern_info["_compiled"].search(header_value):
                    match = CVEMatch(
                        cve_id=pattern_info["cve"],
                        title=pattern_info["title"],
    ...
    async def scan_scripts(self, content: str, url: str = "") -> List[CVEMatch]:
        ...
        scripts = SCRIPT_SOURCE_PATTERN.findall(content)

        for script_src in scripts:
            for pattern, lib_name in VULNERABLE_SCRIPT_PATTERNS:
                match = pattern.search(script_src)
                if match:
                    version = match.group(1)
                    # Check against known vulnerable versions
                    if lib_name in COMPILED_CVE_PATTERNS:
                        for vuln in COMPILED_CVE_PATTERNS[lib_name]:
                            if vuln["_compiled"].search(
                                f"{lib_name}/{version}".lower()
                            ):
                                cve_match = CVEMatch(
                                    cve_id=vuln["cve"],
                                    title=vuln["title"],
    ...
    async def full_scan(
        self, content: str, headers: Dict[str, str], url: str = ""
    ) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        test_headers = {"Server": "Apache/2.4.49", "X-Powered-By": "PHP/5.6.40"}

        def on_finding(f):
            print(f"🔴 {f['cve_id']}: {f['title']} [{f['severity']}]")

        results = await scan_for_cves(
            test_content, test_headers, "https://test.com", on_finding
        )
