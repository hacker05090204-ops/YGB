...
try:
    ...
    AccuracyEngine = None  # type: ignore
    ExecutionStrategy = None  # type: ignore
    PayloadVerificationResult = None  # type: ignore
...
RE_EVENT_HANDLERS = re.compile(
    r'\b(on(?:click|load|error|mouseover|focus|blur|submit|change|input|keydown|keyup|keypress))\s*=\s*["\'][^"\']*["\']',
    re.I,
)
...
RE_IDOR_PATTERNS = (
    (
        re.compile(r"[?&](id|user_id|account_id|order_id|doc_id)=\d+", re.I),
...
RE_SQL_TEST_FORMS = re.compile(r"<form[^>]*>(.*?)</form>", re.I | re.S)
RE_SQL_TEST_INPUTS = re.compile(r'<input[^>]+name=["\']([^"\']+)["\']', re.I)
RE_TEMPLATE_PATTERNS = tuple(
    re.compile(pattern) for pattern in (r"\{\{", r"\{\%", r"\$\{", r"\#\{", r"\<\%")
)
STATIC_EXTENSIONS = (".pdf", ".jpg", ".png", ".css", ".js", ".gif", ".ico")
SQL_ERROR_MARKERS = (
    "sql syntax",
    "mysql_",
    "sqlite_",
...
class WorkflowContext:
    ...
    content_lower: str = ""
    headers_lower: Dict[str, str] = field(default_factory=dict)
    execution_plan: Dict[str, Any] = field(default_factory=dict)
    seen_finding_fingerprints: Set[str] = field(default_factory=set)
    current_phase: int = 0
    stopped: bool = False
...
class UnifiedPhaseRunner:
    ...
    def __init__(
        ...
        self.accuracy_engine = AccuracyEngine() if AccuracyEngine else None
        planned_concurrency = (
            max(1, int(getattr(strategy, "concurrency", 4))) if strategy else 4
        )
        rate_limit = (
            max(1, int(getattr(strategy, "rate_limit_per_host", 2))) if strategy else 2
        )
        self._crawl_semaphore = asyncio.Semaphore(planned_concurrency)
        self._request_delay_seconds = 1.0 / rate_limit
    ...
    async def _emit_action(
        self, action: str, target: str, details: Dict = None, duration_ms: int = 0
    ):
    ...
    def _refresh_context_cache(
        ...
        *,
        ...
        if content is not None:
            context.page_data["content"] = content
            context.content_lower = content.lower()
        if headers is not None:
            context.headers = headers
            context.headers_lower = {
                key.lower(): value for key, value in headers.items()
            }

    def _build_execution_plan(self, context: WorkflowContext) -> Dict[str, Any]:
        if self.strategy is not None:
            return {
                "agent_name": getattr(
                    self.strategy, "agent_name", "workflow-orchestrator"
                ),
                "crawl_depth": getattr(self.strategy, "crawl_depth", 3),
                "concurrency": getattr(self.strategy, "concurrency", 4),
                "payload_profile": getattr(
                    self.strategy, "payload_profile", "standard"
                ),
                "verification_level": getattr(
                    self.strategy, "verification_level", "standard"
                ),
                "priority": getattr(self.strategy, "priority", "medium"),
                "rate_limit_per_host": getattr(self.strategy, "rate_limit_per_host", 2),
                "notes": list(getattr(self.strategy, "notes", [])),
            }
        real_mode = context.mode.upper() == "REAL"
        ...
        return {
            ...
            "rate_limit_per_host": 4 if high_value else 2,
            "notes": ["Local execution plan derived inside phase runner"],
        }

    async def _safe_get(self, url: str) -> tuple[int, str, Dict[str, str]]:
        client = await self._ensure_http_client()
        async with self._crawl_semaphore:
            if self._request_delay_seconds > 0:
                await asyncio.sleep(self._request_delay_seconds)
            response = await client.get(url, headers=DEFAULT_HTTP_HEADERS)
            return response.status_code, response.text, dict(response.headers)

    async def _confirm_reflected_param(
        self,
        base_url: str,
        param_name: str,
    ) -> PayloadVerificationResult:
        ...
        if not query_items:
            return PayloadVerificationResult(
                False, 0.0, ["No query parameters available for reflection probe"]
            )

        marker = f"ygb-reflect-{uuid.uuid4().hex[:10]}"
        rewritten = [
            (name, marker if name == param_name else value)
            for name, value in query_items
        ]
        if not any(name == param_name for name, _ in rewritten):
            return PayloadVerificationResult(
                False, 0.0, [f"Parameter {param_name} not present in probe URL"]
            )

        probe_url = urlunparse(parsed._replace(query=urlencode(rewritten, doseq=True)))
        try:
            _, body, _ = await self._safe_get(probe_url)
            confirmed = marker in body
            notes = [f"Reflection probe executed for parameter {param_name}"]
            if confirmed:
                notes.append("Marker reflected in response body")
            return PayloadVerificationResult(
                confirmed, 0.95 if confirmed else 0.45, notes
            )
        except Exception as exc:
            return PayloadVerificationResult(
                False, 0.0, [f"Reflection probe failed: {exc}"]
            )

    async def _confirm_sqli_candidate(
        self,
        candidate_url: str,
        param_name: str,
    ) -> PayloadVerificationResult:
        ...
        if not query_items:
            return PayloadVerificationResult(
                False, 0.0, ["No query parameters available for SQLi probe"]
            )

        baseline_url = absolute_url
        mutated = []
        changed = False
        for name, value in query_items:
            if name == param_name:
                mutated.append((name, f"{value}'"))
                changed = True
            else:
                mutated.append((name, value))
        if not changed:
            return PayloadVerificationResult(
                False, 0.0, [f"Parameter {param_name} not present in candidate URL"]
            )

        probe_url = urlunparse(parsed._replace(query=urlencode(mutated, doseq=True)))
        try:
            _, baseline_body, _ = await self._safe_get(baseline_url)
            _, probe_body, _ = await self._safe_get(probe_url)
            baseline_lower = baseline_body.lower()
            probe_lower = probe_body.lower()
            new_errors = [
                marker
                for marker in SQL_ERROR_MARKERS
                if marker in probe_lower and marker not in baseline_lower
            ]
            confirmed = bool(new_errors)
            notes = [f"SQLi probe executed for parameter {param_name}"]
            if new_errors:
                notes.append(
                    f"New SQL error markers observed: {', '.join(new_errors[:3])}"
                )
            return PayloadVerificationResult(
                confirmed, 0.97 if confirmed else 0.5, notes
            )
        except Exception as exc:
            return PayloadVerificationResult(False, 0.0, [f"SQLi probe failed: {exc}"])
    ...
    async def _add_finding(
        ...
        if self.accuracy_engine is not None:
            verification_counters = context.governance_data.setdefault(
            ...
            outcome = self.accuracy_engine.verify(
                category=category,
                severity=severity,
                title=title,
                ...
                seen_fingerprints=context.seen_finding_fingerprints,
                prior_findings=[
                    {
                        "category": finding.category,
                        "title": finding.title,
                    ...
                    for finding in context.findings[-25:]
                ],
                strategy_name=str(
                    context.execution_plan.get("agent_name")
                    or getattr(self.strategy, "agent_name", "crawler-agent")
                ),
                task_type="workflow",
            )
            if outcome.duplicate:
                verification_counters["duplicates"] = (
                    int(verification_counters.get("duplicates", 0)) + 1
                )
                return
            if outcome.status == "REJECTED_FALSE_POSITIVE":
                verification_counters["rejected_false_positives"] = (
                    int(verification_counters.get("rejected_false_positives", 0)) + 1
                )
                return
    ...
    async def run_workflow(
        self, target_url: str, workflow_id: str, mode: str = "READ_ONLY"
    ) -> WorkflowContext:
        """Run complete security analysis with ALL phases."""
        context = WorkflowContext(
            target_url=target_url, workflow_id=workflow_id, mode=mode
        )
        context.execution_plan = self._build_execution_plan(context)
        ...
        # ==========================================================
        await self._run_phase(
            context, 9, "CSRF Token Analysis", 45, lambda c: self._phase_check_csrf(c)
        )
        ...
        max_pages = max(
            1,
            int(context.execution_plan.get("crawl_depth", 5 if mode == "REAL" else 3)),
        )
        await self._run_phase(
            context,
            13,
            f"Crawling Links (max {max_pages})",
            60,
        ...
        # ==========================================================
        await self._run_phase(
            context, 17, "CORS Policy Check", 80, lambda c: self._phase_cors_check(c)
        )
        ...
        # ==========================================================
        await self._run_phase(
            context, 18, "CSP Analysis", 85, lambda c: self._phase_csp_analysis(c)
        )
        ...
        # ==========================================================
        await self._run_phase(
            context, 19, "Capturing Screenshot", 90, lambda c: self._phase_screenshot(c)
        )
        ...
        # ==========================================================
        await self._run_phase(
            context, 35, "JWT Token Analysis", 72, lambda c: self._phase_jwt_analysis(c)
        )
        ...
        if context.stopped:
            context.phase_results.append(
                PhaseResult(
                    ...
                    status="SKIPPED",
                    duration_ms=0,
                    output={"skipped": True, "reason": "workflow_stopped"},
                )
            )
            return
        await self._emit(
    ...
    async def _phase_governance_init(self, context: WorkflowContext) -> Dict:
        ...
        for i in range(1, 20):
            try:
                module_path = (
                    f"python.phase{i:02d}_core"
                    if i == 1
                    else f"python.phase{i:02d}_actors"
                    if i == 2
                    else None
                )
                if module_path:
    ...
    async def _phase_navigate(self, context: WorkflowContext) -> Dict:
        ...
        if self.use_browser and self.driver:
            ...
            self._refresh_context_cache(
                context, headers=dict(response.headers), content=response.text
            )
            ...
            await self._emit_action(
                "NAVIGATE", url, {"status": response.status_code}, duration
            )
    ...
    async def _phase_extract_content(self, context: WorkflowContext) -> Dict:
        ...
        if self.use_browser and self.driver:
            ...
            context.links = [
                l.get_attribute("href") for l in links[:100] if l.get_attribute("href")
            ]
        else:
            html = context.page_data.get("content", "")
            context.links = RE_HREF.findall(html)[:100]

        self._refresh_context_cache(context, content=html, headers=context.headers)
        ...
        await self._emit_action(
            "EXTRACT",
            context.page_data.get("url", context.target_url),
            {"links": len(context.links), "size": len(html)},
        )
    ...
    async def _phase_detect_forms(self, context: WorkflowContext) -> Dict:
        ...
        if self.use_browser and self.driver:
            ...
            form_matches = re.findall(
                r"<form[^>]*>(.*?)</form>", content, re.I | re.DOTALL
            )
        ...
        await self._emit_action(
            "FORM_DETECT", context.target_url, {"forms": len(forms)}
        )
    ...
    async def _phase_analyze_headers(self, context: WorkflowContext) -> Dict:
        ...
        headers = context.headers_lower or {
            k.lower(): v for k, v in context.headers.items()
        }
        findings = 0
    ...
    async def _phase_detect_xss(self, context: WorkflowContext) -> Dict:
        ...
        content = context.page_data.get("content", "")
        content_lower = context.content_lower or content.lower()
        findings = 0

        # Check for inline event handlers
        event_handlers = RE_EVENT_HANDLERS.findall(content)
        if len(event_handlers) >= 3:
            await self._add_finding(
                context,
                "XSS",
                "LOW",
                ...
                evidence={
                    ...
                    "inline_event_handlers": len(event_handlers),
                },
            )
            findings += 1
        ...
        if parsed.query:
            ...
            if reflected_params:
                verification = await self._confirm_reflected_param(
                    context.target_url,
                    reflected_params[0],
                )
                await self._add_finding(
                    ...
                    f"Reflected parameters detected: {', '.join(reflected_params)}",
                    evidence={
                        "reflected_parameters": reflected_params,
                        "payload_tested": True,
                        "response_validated": verification.confirmed,
                        "exploit_confirmed": verification.confirmed,
                        "verification_notes": verification.notes,
                    },
                )
                findings += len(reflected_params)
        ...
        for sink in dom_sinks:
            if sink in content:
                await self._add_finding(
                    ...
                    evidence={"needs_manual_review": True, "dom_sink": sink},
                )
                findings += 1

        await self._emit_action(
            "XSS_SCAN", context.target_url, {"patterns_found": findings}
        )
    ...
    async def _phase_detect_sqli(self, context: WorkflowContext) -> Dict:
        ...
        content = context.content_lower or context.page_data.get("content", "").lower()
        findings = 0
        ...
        for pattern, desc in sql_errors:
            if pattern in content:
                ...
                await self._add_finding(
                    ...
                    evidence={"sql_errors": matched_errors[:], "payload_tested": True},
                )
                findings += 1
        ...
        for link in context.links[:50]:
            for param in sqli_params:
                if f"{param}=" in link:
                    if probes_used < 2:
                        verification = await self._confirm_sqli_candidate(
                            urljoin(context.target_url, link),
                            param,
                        )
                        probes_used += 1
                    else:
                        verification = PayloadVerificationResult(
                            False,
                            0.0,
                            ["SQLi probe skipped to limit verification load"],
                        )
                    await self._add_finding(
                        context,
                        "SQLI",
                        ...
                        evidence={
                            "needs_manual_review": not verification.confirmed,
                            "parameter": param,
                            "candidate_url": link[:100],
                            "payload_tested": True,
                            "response_validated": verification.confirmed,
                            "exploit_confirmed": verification.confirmed,
                            "verification_notes": verification.notes,
    ...
    async def _phase_check_csrf(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "CSRF_CHECK",
            context.target_url,
            {"forms": len(context.forms), "findings": findings},
        )
    ...
    async def _phase_detect_idor(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "IDOR_CHECK", context.target_url, {"findings": findings}
        )
    ...
    async def _phase_info_disclosure(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "INFO_SCAN",
            context.target_url,
            {"comments": len(comments), "findings": findings},
        )
    ...
    async def _phase_detect_tech(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "TECH_DETECT", context.target_url, {"technologies": techs}
        )

        return {"technologies": techs}

    async def _phase_crawl_links(
        self, context: WorkflowContext, max_pages: int
    ) -> Dict:
        ...
        for link in context.links:
            ...
            if parsed.netloc == parsed_base.netloc and full_url not in visited:
                if not any(
                    ext in parsed.path
                    for ext in [".pdf", ".jpg", ".png", ".css", ".js", ".gif", ".ico"]
                ):
        ...
        discovered_pages = []
        sql_tested = 0
        selected_urls = [url for url in to_visit if url not in visited][:max_pages]
        ...
        if self.use_browser and self.driver:
            for url in selected_urls:
                try:
                    ...
                    if any(
                        marker in url.lower()
                        for marker in ["/login", "/search", "/admin", "/user", "/api"]
                    ):
            ...
            async def fetch_candidate(candidate_url: str):
                async with self._crawl_semaphore:
                    if self._request_delay_seconds > 0:
                        await asyncio.sleep(self._request_delay_seconds)
                    try:
                        response = await client.get(
                            candidate_url, headers=DEFAULT_HTTP_HEADERS
                        )
                        return (
                            candidate_url,
                        ...
                        return candidate_url, 0, "", {}

            results = await asyncio.gather(
                *(fetch_candidate(url) for url in selected_urls)
            )
            for url, status_code, html, resp_headers in results:
        ...
        await self._emit_action(
            ...
            context.target_url,
            {
                ...
                "planned_depth": max_pages,
                "concurrency": context.execution_plan.get("concurrency", 1),
            },
            duration,
        )
    ...
    async def _test_sql_injection(self, context: WorkflowContext, url: str, html: str):
        ...
        forms = RE_SQL_TEST_FORMS.findall(html)
        ...
        for form in forms:
            inputs = RE_SQL_TEST_INPUTS.findall(form)
            if inputs:
                await self._add_finding(
                    context,
                    "SQLI",
    ...
    async def _quick_scan(
        self, context: WorkflowContext, url: str, html: str, headers: Dict
    ):
        ...
        if "x-frame-options" not in headers_lower:
            await self._add_finding(
                ...
                f"Page {url} vulnerable to clickjacking",
                url,
            )
        ...
        if "content-security-policy" not in headers_lower:
            await self._add_finding(
                ...
                "MEDIUM",
                "Missing CSP",
                f"Page {url} lacks Content-Security-Policy",
                url,
            )
    ...
    async def _phase_js_analysis(self, context: WorkflowContext) -> Dict:
        ...
        for pattern, desc in RE_SENSITIVE_JS:
            if pattern.search(content):
                await self._add_finding(
                    ...
                    evidence={"payload_tested": True, "matched_pattern": desc},
                )
                findings += 1
    ...
    async def _phase_api_discovery(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "API_SCAN", context.target_url, {"endpoints": len(endpoints)}
        )
    ...
    async def _phase_auth_analysis(self, context: WorkflowContext) -> Dict:
        ...
        has_login = any(
            "password" in str(f.get("inputs", [])).lower() for f in context.forms
        )
        ...
        await self._emit_action(
            "AUTH_SCAN", context.target_url, {"has_login": has_login}
        )
    ...
    async def _phase_cors_check(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "CORS_CHECK", context.target_url, {"findings": findings}
        )
    ...
    async def _phase_csp_analysis(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "CSP_CHECK",
            context.target_url,
            {"has_csp": bool(csp), "findings": findings},
        )
    ...
    async def _phase_subdomain_enum(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "SUBDOMAIN_ENUM", domain, {"checking": len(common_subdomains)}
        )
    ...
    async def _phase_ssl_check(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "SSL_CHECK", context.target_url, {"https": is_https, "findings": findings}
        )
    ...
    async def _phase_open_redirect(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "REDIRECT_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_ssrf_detection(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "SSRF_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_xxe_detection(self, context: WorkflowContext) -> Dict:
        ...
        if (
            ...
            or "application/xml" in str(context.headers).lower()
        ):
            await self._add_finding(
                context,
                "XXE",
    ...
    async def _phase_path_traversal(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "PATH_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_file_inclusion(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "FILE_INCLUSION", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_template_injection(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "SSTI_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_clickjacking(self, context: WorkflowContext) -> Dict:
        ...
        existing_clickjacking = any(
            finding.category in {"HEADERS", "CLICKJACKING"}
            and "frame" in finding.title.lower()
            for finding in context.findings
        )

        if not has_xfo and not has_csp_frame and not existing_clickjacking:
            await self._add_finding(
                context,
                "CLICKJACKING",
                "MEDIUM",
    ...
    async def _phase_http_methods(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "HTTP_METHODS", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_header_injection(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "HEADER_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_websocket_security(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "WEBSOCKET_CHECK",
            context.target_url,
            {"has_ws": "websocket" in content.lower()},
        )
        return {"findings": findings}
    ...
    async def _phase_graphql_security(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "GRAPHQL_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_oauth_security(self, context: WorkflowContext) -> Dict:
        ...
        has_oauth = any(p in content for p in oauth_providers) and (
            "login" in content or "signin" in content
        )
        ...
        await self._emit_action(
            "OAUTH_CHECK", context.target_url, {"has_oauth": has_oauth}
        )
    ...
    async def _phase_rate_limiting(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "RATE_CHECK", context.target_url, {"has_rate_limit": has_rate_limit}
        )
    ...
    async def _phase_captcha_check(self, context: WorkflowContext) -> Dict:
        ...
        has_captcha = (
            "captcha" in content or "recaptcha" in content or "hcaptcha" in content
        )
        has_login = any(
            "password" in str(f.get("inputs", [])).lower() for f in context.forms
        )
        ...
        await self._emit_action(
            "CAPTCHA_CHECK", context.target_url, {"has_captcha": has_captcha}
        )
    ...
    async def _phase_payment_security(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "PAYMENT_CHECK", context.target_url, {"has_payment": has_payment}
        )
    ...
    async def _phase_business_logic(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "LOGIC_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_race_conditions(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "RACE_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_password_policy(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "PASSWORD_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_2fa_analysis(self, context: WorkflowContext) -> Dict:
        ...
        has_2fa = (
            "2fa" in content
            or "two-factor" in content
            or "authenticator" in content
            or "otp" in content
        )
    ...
    async def _phase_session_fixation(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "SESSION_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_privilege_escalation(self, context: WorkflowContext) -> Dict:
        ...
        for link in context.links[:50]:
            if (
                "admin" in link.lower()
                or "role=" in link.lower()
                or "isadmin" in link.lower()
            ):
                await self._add_finding(
                    context,
                    "PRIVESC",
        ...
        await self._emit_action(
            "PRIVESC_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_data_exposure(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "DATA_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_cve_scan(self, context: WorkflowContext) -> Dict:
        ...
        if CVE_SCANNER_AVAILABLE:
            try:
                ...
                results = await scan_for_cves(
                    content, headers, context.target_url, on_cve
                )
        ...
        await self._emit_action(
            "CVE_SCAN", context.target_url, {"findings": cve_findings}
        )
    ...
    async def _phase_dependency_scan(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "DEPENDENCY_CHECK", context.target_url, {"findings": findings}
        )
        return {"findings": findings}
    ...
    async def _phase_final_scan(self, context: WorkflowContext) -> Dict:
        ...
        await self._emit_action(
            "FINAL_SCAN", context.target_url, {"total_findings": total}
        )
    ...
    async def _phase_generate_report(self, context: WorkflowContext) -> Dict:
        ...
        report = {
            ...
            "candidate_findings": len(pending_findings),
        ...
        lines.append("-" * 80)
        lines.append(f"Verified Findings: {len(final_findings)}")
        lines.append(f"Pending Verification: {len(pending_findings)}")
        lines.append(f"Pages Analyzed: {len(context.pages_visited)}")
        lines.append(f"Phases Executed: {len(context.phase_results)}")
        lines.append(
            f"Technologies: {', '.join(context.technologies) or 'None detected'}"
        )
        lines.append("")
        lines.append("-" * 80)
        ...
        for severity in ["CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"]:
            severity_findings = [
                f for f in final_findings if f.severity.upper() == severity
            ]
        ...
        if pending_findings:
            ...
            lines.append("=" * 80)
            lines.append("")
            for finding in pending_findings[:25]:
                lines.append(f"[{finding.finding_id}] {finding.title}")
                lines.append(f"  Severity: {finding.severity}")
                lines.append(f"  Confidence: {finding.confidence:.2f}")
                lines.append(f"  Verification: {finding.verification_status}")
        ...
        for phase in context.phase_results:
            ...
            lines.append(
                f"  [{status}] Phase {phase.phase_number}: {phase.phase_name} ({phase.duration_ms}ms)"
            )
        ...
        await self._emit_action(
            "REPORT", context.target_url, {**report, "report_file": str(report_path)}
        )
...
async def run_real_workflow(
    ...
    runner = UnifiedPhaseRunner(
        on_progress=on_progress, http_client=http_client, strategy=strategy
    )
    ...
    async def test():
        def on_progress(update):
            ...
            if t == "phase_start":
                ...
                print(
                    f"{icon} Phase {update.get('phase')}: {status} ({update.get('duration_ms')}ms)"
                )
