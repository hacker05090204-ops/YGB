"""Phase-21 Forbidden Import Tests."""
import pytest
from pathlib import Path

PHASE21_DIR = Path(__file__).parent.parent
IMPLEMENTATION_FILES = ["phase21_types.py", "phase21_context.py", "phase21_engine.py", "__init__.py"]


def _read_source(filename: str) -> str:
    filepath = PHASE21_DIR / filename
...
class TestForbiddenImports:
    ...
    def test_no_future_phase_imports(self, filename: str) -> None:
        ...
        for phase in range(22, 35):
            assert f"phase{phase}" not in source


class TestNoFileOperations:
    @pytest.mark.parametrize("filename", ["phase21_types.py", "phase21_context.py", "phase21_engine.py"])
