    ...
    def _create_sample(self, idx: int, label: int, is_edge: bool) -> Sample:
        ...
        if label == 1:
            if is_edge:
                features = {
                    "type": "edge_case",
                    "difficulty": self.rng.uniform(0.72, 0.98),
                    "noise": self.rng.uniform(0.22, 0.42),
                }
            else:
                features = {
                    "type": "standard",
                    "difficulty": self.rng.uniform(0.48, 0.82),
                    "noise": self.rng.uniform(0.04, 0.18),
                }
        else:
            if is_edge:
                features = {
                    "type": "edge_case",
                    "difficulty": self.rng.uniform(0.28, 0.64),
                    "noise": self.rng.uniform(0.16, 0.36),
                }
            else:
                features = {
                    "type": "standard",
                    "difficulty": self.rng.uniform(0.08, 0.42),
                    "noise": self.rng.uniform(0.0, 0.14),
                }
