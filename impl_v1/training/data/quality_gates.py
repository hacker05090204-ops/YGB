...
def check_class_imbalance(
    labels: List[int], max_ratio: float = 0.60, min_ratio: float = 0.40
) -> GateResult:
    """
...
def check_duplicates(
    features: np.ndarray, exact_threshold: float = 0.01, near_threshold: float = 0.05
) -> GateResult:
    """
    ...
    # Keep it deterministic so production verification never depends on RNG.
    max_check = min(n, 2000)
    sample_idx = (
        ((np.arange(max_check, dtype=np.int64) * n) // max_check)
        if n > max_check
        else np.arange(n)
    )
...
def check_regression_gate(
    ...
    for metric, current in current_metrics.items():
        ...
        if "loss" in metric.lower():
            if current > baseline * (1 + regression_threshold):
                regressions.append(f"{metric}: {current:.4f} > baseline {baseline:.4f}")
        else:
    ...
    return GateResult(
        "regression", True, "INFO", "No regression vs baseline", current_metrics
    )
...
def check_promotion_gate(
    ...
    if verified_samples < threshold:
        ...
        issues.append(
            f"Insufficient samples: {verified_samples}/{threshold} (deficit: {deficit})"
        )
    ...
    return GateResult(
        ...
        {"verified_samples": verified_samples, "threshold": threshold},
    )
...
def run_all_gates(
    ...
    results.append(check_regression_gate(current_metrics or {}, baseline_metrics))
    results.append(
