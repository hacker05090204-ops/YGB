"""
Phase-26 Forbidden Import Tests.

Tests that FAIL HARD if any forbidden imports appear in source code.
"""
import pytest
from pathlib import Path


PHASE26_DIR = Path(__file__).parent.parent
...
class TestForbiddenImports:
    ...
    def test_no_future_phase_imports(self, filename: str) -> None:
        ...
        assert "phase27" not in source
        assert "phase28" not in source
        assert "phase29" not in source
        assert "phase30" not in source
        assert "phase31" not in source
        assert "phase32" not in source
        assert "phase33" not in source
...
class TestNoFileOperations:
    ...
    @pytest.mark.parametrize("filename", ["phase26_types.py", "phase26_context.py", "phase26_engine.py"])
